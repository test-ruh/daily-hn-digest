#!/usr/bin/env bash
# =============================================================================
# One-command setup for Hacker News Daily Digest
...
AGENT_NAME="Hacker News Daily Digest"
AGENT_ID="hacker-news-daily-digest"
...
phase_db() {
  ...
  if [[ -n "${PG_CONNECTION_STRING:-}" ]]; then
    ...
    if ${local_pg}; then
      ...
      warn "     Then set in .env: PG_CONNECTION_STRING=postgresql://postgres:<choose-a-password>@localhost:5432/hacker-news-daily-digest"
      exit 1
    fi
...
phase_finish() {
  ...
  if command -v openclaw >/dev/null 2>&1; then
    if openclaw cron list 2>/dev/null | grep -q "^hourly-hacker-news-fetch\b"; then
      ok "cron 'hourly-hacker-news-fetch' already registered"
    else
      openclaw cron add --file cron/hourly-hacker-news-fetch.json && ok "registered cron 'hourly-hacker-news-fetch'"
    fi
  else
    warn "openclaw CLI not found — register manually: openclaw cron add --file cron/hourly-hacker-news-fetch.json"
  fi
  ...
  log "  openclaw cron run --name hourly-hacker-news-fetch    # trigger manually"
